...
if (isset($_FILES['excel']) && $_FILES['excel']['error'] === UPLOAD_ERR_OK) {
    ...
    try {
        ...
        foreach ($cantidad as $index => $row) {
            if (!empty($row[0])) {
                // Escapar los valores
                $Codigo_Item = mysqli_real_escape_string($mysqli, $row[0]);
                $Descripcion_Item = mysqli_real_escape_string($mysqli, $row[1]);

                // Crear consulta
                $consulta = "INSERT INTO cpms (Codigo_Item , Descripcion_Item) 
                            VALUES ('$row[0]', '$row[1]')";
