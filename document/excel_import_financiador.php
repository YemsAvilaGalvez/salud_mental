...
if (isset($_FILES['excel']) && $_FILES['excel']['error'] === UPLOAD_ERR_OK) {
    ...
    try {
        ...
        foreach ($cantidad as $index => $row) {
            if (!empty($row[0])) {
                // Escapar los valores
                $Id_Financiador = mysqli_real_escape_string($mysqli, $row[0]);
                $Descripcion_Financiador = mysqli_real_escape_string($mysqli, $row[1]);

                // Crear consulta
                $consulta = "INSERT INTO financiador (Id_Financiador, Descripcion_Financiador) 
                             VALUES ('$row[0]', '$row[1]')";
